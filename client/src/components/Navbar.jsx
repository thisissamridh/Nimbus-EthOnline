  ...
  return (
    <>
      ...
      >
        <NavLink
          to="/"
          className=" text-2xl w-full pt-2 h-fit flex justify-start items-center py-2 ml-24 font-bold text-green-500 font-mono"
        ...
        {account.currentAccount == null ? (
          ...
          <div className="w-1/3 flex justify-center items-center mr-24 text-white font-bold">
            Hey 👋{" "}
            {`${String(account.currentAccount).slice(0, 4)}...${String(
              account.currentAccount
            ).slice(String(account.currentAccount).length - 4)}`}
          </div>
        )}
      </div>
    </>
